...
| `monitor_act_aligned_training.py` | current diagnostic | Read-only ACT-aligned training monitor with step progress, measured throughput, ETA, loss/memory summaries, checkpoint state, validation state, and stale-log warnings. | ACT-aligned trainer | training output directory | terminal progress report | no | no | no |
