...
| `train_official_act.py` | current | Train the faithful force-free Official ACT Motion-CVAE baseline. | Official ACT | HDF5 data root, optional experiment manifest/resume checkpoint | resume-capable periodic/best/final checkpoints and JSONL metrics | no | no | optional |
| `train_official_act_no_latent.py` | current | Train Official ACT with the posterior, latent projection and latent token structurally removed. | Official ACT NoLatent | HDF5 data root, optional experiment manifest/resume checkpoint | resume-capable periodic/best/final checkpoints and JSONL metrics | no | no | optional |
| `preflight_official_act_no_latent.py` | diagnostic | Audit the structural contract and one real-data optimization step before NoLatent training. | Official ACT NoLatent | HDF5 data root | JSON report to stdout and optional file | no | no | optional |
...
The no-resume limitation applies to the legacy trainers above. The three ACT-aligned native-rate trainers have strict mid-epoch resume, RNG/DataLoader-state restoration, and checkpoint reload auditing. The two Official ACT trainers resume at completed epoch boundaries with strict model/training config and RNG restoration. See [`../training/NATIVE_RATE_CONTROL_MODELS.md`](../training/NATIVE_RATE_CONTROL_MODELS.md) and [`../training/OFFICIAL_ACT_NO_LATENT.md`](../training/OFFICIAL_ACT_NO_LATENT.md).
...
| `monitor_act_aligned_training.py` | current diagnostic | Read-only ACT-aligned and Official ACT training monitor with step progress, measured throughput, ETA, loss/memory summaries, checkpoint state, validation state, and stale-log warnings. | ACT-aligned or Official ACT trainer | training output directory | terminal progress report | no | no | no |
