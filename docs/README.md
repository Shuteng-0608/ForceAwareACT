...
- [`PAIRED50_Q1_TEMPORAL_ROLLOUT_PILOT_REPORT.md`](experiments/PAIRED50_Q1_TEMPORAL_ROLLOUT_PILOT_REPORT.md) records the corresponding fixed-point closed-loop pilot, including the Contact-CVAE `k=0.04~0.10` sampled continuous-success region and its safety limits.
- [`HOLE_FIXED_SCRIPTED50_ACT_CONTACT_CVAE_EXPERIMENT_REPORT.md`](experiments/HOLE_FIXED_SCRIPTED50_ACT_CONTACT_CVAE_EXPERIMENT_REPORT.md) records the new scripted-50 dataset, paired 25,000-step training runs, fixed-center signed-`k` studies, and the completed three-`k` Fibonacci 4 mm rollout comparison.
- [`PEG_HOLE_100_PAIRED50_ACT_CONTACT_CVAE_STAGE_REPORT.md`](experiments/PEG_HOLE_100_PAIRED50_ACT_CONTACT_CVAE_STAGE_REPORT.md) consolidates the paired-50 `peg_hole_100` training runs, holdout evaluation, fixed-Q=1 signed-`k` scans, fixed-center rollouts, and the completed 1,000-run Fibonacci 4 mm comparison.
