"""Versioned checkpoints for latent-free official ACT."""
...
OFFICIAL_ACT_NO_LATENT_CHECKPOINT_VERSION = (
    "official_act_no_latent_checkpoint_v1"
)


def save_official_act_no_latent_checkpoint(
    path: Path,
    *,
    ...
    payload = {
        "format_version": OFFICIAL_ACT_NO_LATENT_CHECKPOINT_VERSION,
...
def read_official_act_no_latent_checkpoint(
    path: Path,
    *,
    ...
    if (
        ...
        raise ValueError("unsupported latent-free official ACT checkpoint")
    return payload


def load_official_act_no_latent_checkpoint(
    path: Path,
    *,
    model: OfficialACTNoLatentPolicy,
    optimizer: torch.optim.Optimizer,
...
def construct_official_act_no_latent_from_checkpoint(
    path: Path,
    *,
    map_location: Any,
) -> tuple[
    ...
    model = OfficialACTNoLatentPolicy(
        OfficialACTNoLatentConfig(**payload["model_config"])
    ).to(map_location)
    config = OfficialACTNoLatentTrainingConfig(
        **payload["training_config"]
    )
    optimizer = build_official_act_no_latent_optimizer(model, config)
