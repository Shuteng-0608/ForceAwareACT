"""Training configuration for the latent-free official ACT baseline."""
...
OFFICIAL_ACT_NO_LATENT_TRAINING_VERSION = (
    "official_act_no_latent_training_v1"
)


@dataclass(frozen=True)
class OfficialACTNoLatentTrainingConfig:
    training_version: str = OFFICIAL_ACT_NO_LATENT_TRAINING_VERSION
    ...
    selection_metric: str = "no_latent_validation_action_l1"

    def __post_init__(self) -> None:
        if self.training_version != OFFICIAL_ACT_NO_LATENT_TRAINING_VERSION:
            raise ValueError(
                "training_version must be "
                f"{OFFICIAL_ACT_NO_LATENT_TRAINING_VERSION!r}"
            )
        for name in (
            "learning_rate",
        ...
        if self.selection_metric != "no_latent_validation_action_l1":
            raise ValueError(
