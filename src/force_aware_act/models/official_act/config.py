...
OFFICIAL_ACT_NO_LATENT_ARCHITECTURE_VERSION = (
    "official_act_single_arm_no_latent_v1"
)
...
@dataclass(frozen=True)
class OfficialACTNoLatentConfig:
    """Official ACT decoder hyperparameters without a latent mechanism."""

    architecture_version: str = OFFICIAL_ACT_NO_LATENT_ARCHITECTURE_VERSION
    d_model: int = 512
    nhead: int = 8
    dim_feedforward: int = 3200
    encoder_layers: int = 4
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            raise ValueError(
                "architecture_version must be "
                f"{OFFICIAL_ACT_NO_LATENT_ARCHITECTURE_VERSION!r}"
            )
        if self.d_model % self.nhead != 0 or self.d_model % 4 != 0:
            raise ValueError("d_model must be divisible by nhead and by 4")
        if not 0.0 <= self.dropout < 1.0:
    ...
    @classmethod
    def canonical(cls, **overrides: Any) -> "OfficialACTNoLatentConfig":
        return cls(**overrides)

    @classmethod
    def compact_smoke(cls, **overrides: Any) -> "OfficialACTNoLatentConfig":
        values: dict[str, Any] = {
            "d_model": 32,
            "nhead": 4,
            "dim_feedforward": 64,
    ...
    @property
    def policy_memory_token_count(self) -> int:
        return self.visual_token_count + 1
    ...
    def checkpoint_metadata(self) -> dict[str, Any]:
        ...
        metadata.update(
            {
                "source_implementation": "/home/stw/act",
                "architecture_derivation": "official_act_minus_motion_latent",
                "policy_encoder_layers": self.encoder_layers,
                "policy_decoder_layers": self.decoder_layers,
                "camera_feature_layout": "concatenate_feature_width",
                "policy_memory_layout": "qpos_visual",
                "latent_mechanism": "none",
                "deployment_latent": "none",
                "visual_token_count": self.visual_token_count,
                "policy_memory_token_count": self.policy_memory_token_count,
            }
        )
