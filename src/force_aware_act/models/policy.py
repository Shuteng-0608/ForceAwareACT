"""Minimal ForceAwareACT policy forward pass."""
...
from force_aware_act.models.posterior import ContactPosteriorEncoder, MotionPosteriorEncoder
from force_aware_act.models.vision import ResNet18VisionEncoder


class JointMLP(nn.Module):
    """Encode current joint positions into one policy token."""

    def __init__(self, q_dim: int = 7, d_model: int = 512) -> None:
        super().__init__()
        if q_dim <= 0:
    ...
    def forward(self, qpos: torch.Tensor) -> torch.Tensor:
        ...
        if qpos.shape[1] != self.q_dim:
            raise ValueError(f"qpos last dimension must be {self.q_dim}, got {qpos.shape[1]}")
        return self.net(qpos)


class ForceAwareACTPolicy(nn.Module):
    """Minimal contact-dynamics-aware ACT policy composition.

    Training may use future action and force chunks to sample posterior latents.
    Inference only accepts online inputs and uses zero motion/contact latents.
    """
